...
export const useContestStore = defineStore('contest', {
  ...
  actions: {
    ...
    async login(passwd) {
      if (this.api.length === 0)
        return { code: -1, msg: 'Service API cannot be empty' }
      ...
      try {
        ...
        return { code: -1, msg: 'Fail to login: ' + err.message }
      }
    },

    // Refresh
    async refresh() {
      if (this.lastRefresh === null || new Date().getTime() - this.lastRefresh < 86400)
        return { code: 0 }
      ...
      try {
        ...
        return { code: -1, msg: 'Fail to refresh: ' + err.message }
      }
    },
  ...
  persist: {
    paths: ['api', 'userId', 'lastRefresh', 'accessToken', 'refreshToken']
  }
})
